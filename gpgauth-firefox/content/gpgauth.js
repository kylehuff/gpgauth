...
var gpgAuth = {
    ...
    */
    onLoad: function() {
        ...
        this.strings = document.getElementById( "gpgauth-strings" );

        this.gpg_elements = new Array();
        this.prefs = Components.classes["@mozilla.org/preferences-service;1"].
                getService(Components.interfaces.nsIPrefService);

        this.gpgAuthPlugin = document.getElementById('gpgAuthPlugin');

        this.prefs = this.prefs.getBranch("extensions.gpgauth");
    ...
    This function is called when a menu item is selected. It calls <onDelayMenuAction> after a 100 milliseconds later.
    */
    onMenuItemCommand: function(e,action) {
        setTimeout("gpgAuth.onDelayMenuAction('"+action+"')", 100);
    },

    /*
    Function: onDelayMenuAction
    This function is called to exectue an action. It forwards the the call to the correct function in main class, or load the requested dialog.
    ...
    */
    doServerTests: function( STK_ELM, UTK_ELM, event ) {
        ...
        // Call gpgAuth.crypt specifying the domain as the GPG Key to encrypt to.
        ...
        var server_token = gpgAuth.crypt( true, gpgAuth.gpg_elements[ gpgAuth.domain ][ 'RANDOM_VALUE' ], [ gpgAuth.domain ], true );
        ...
        if ( server_token.result != gpgAuth.Const.Results.SUCCESS ) {
    ...
    */
    decryptUserToken: function( UTK_ELM, e ) {
        ...
        if ( user_token && ms < 300000 ) {
            ...
            var result = gpgAuth.decrypt( true, user_token );
            if ( result.result == gpgAuth.Const.Results.SUCCESS ) {
                ...
                if ( random_re.test( result.decrypted ) ) {
                    ...
                    alert( "The server did not provide a token compatible with this version of gpgAuth; Aborting." );
                    return false;
                }
            } else {
    ...
    */
    ...
    gpgauthDialog: function ( message_type, error_message, details, check_text ) {
        ...
        if ( message_type == "warning" ) {
            ...
            var icon = "chrome://gpgauth/skin/Warning.png";
        } else if ( message_type == "error" ) {
            var title = "gpgAuth: Error";
            var icon = "chrome://gpgauth/skin/Error.png";
        } else {
            var title = "gpgAuth: Message";
            var icon = "chrome://gpgauth/skin/Question.png";
        }
        ...
        window.openDialog( "chrome://gpgauth/content/gpgauth_dialog.xul", "gpgauthDialog", "chrome, dialog, modal, centerscreen", params ).focus();
        return params.pout;
    }
};
